                    ...
                    if D.X == 1: # Zu beschäftigt für eine Unterhaltung-Option
                        ...
                        if D.X == 1:# Kommentarlos nach oben gehen
                            ...
                            if D.X == 1:#Chef anschreien
                                ...
                                D.clean_print(f"Jetzt kommt dir der Gedanke, dich mit dem Chef anzulegen, du willst dich doch nicht wegen eines Minijobs so erniedrigen lassen oder? \n[1 = erniedrigen lassen],[2 = kämpfen]")
                                D.get_choice(2)
                                if D.X == 1:
                                    D.clean_print("Du bleibst vollkommen gedemütigt und geschwächt liegen, während der Chef in einen myteriösen Gang verschwindet.")
                                    Social_Credits -=15
                                    ...
                                    D.clean_print("\nAuch hast du 20 Lebenspunkte vor dem Schock verloren.")
                                    Spieler_HP -= 20
                                    D.clean_print("Drücke eine beliebige Taste, um fortzufahren..")
                                    warte = input()
                                    D.clear_screen()
                                ...
                                elif D.X == 2: #Kampf mit Filialleiter
                                    D.clean_print("Drücke eine beliebige Taste, um fortzufahren..")
                                    warte = input()
                                    D.clear_screen()
                                    D.clean_print(f"Mutig, von Wut und Schmerz geleitet, stehst du auf und willst diesem Mann zeigen, wer du wirklich bist.\nDein Chef deutet dich sofort und begibt sich in Kampfstellung. Ihr werdet jetzt kämpfen.")
                                    D.Kampf(Spieler_HP,Filialleiter_HP,False)
                                    if D.Win == 0:
                                        Social_Credits -= 15
                                        D.clean_print(f"\nDu hast gerade 15 Social Credits verloren. Es sieht ganz schlecht für dich aus.\n Wenn du zu viele Punkte verlierst, kann es zu schweren Konsequenzen führen.\n Dir bleiben noch {Social_Credits} übrig.")
                                        D.clean_print(f"\nAuch hast du nur noch {Spieler_HP} Lebenspunkte übrig.")
                                        D.clean_print("Drücke eine beliebige Taste, um fortzufahren..")
                                        warte = input()                                        
                                    elif D.Win == 1:
                                        D.clean_print(f"Voller Freude und Rachelust schaust du auf deinen Chef herab und merkst, dass er dir was sagen will.\nDu beugst dich nieder und hörst wie er sagt:\n\n{Blau}'Wach auf, wach aufff, bist du daa'{Reset}\n\nPlötzlich wird dir ganz komisch und du bist ganz verwirrt doch dann öffnest du deine Augen und dein Mitarbeiter Karl steht vor dir.\nOh nein..Es scheint, als hättest du doch nicht gewonnen, sondern in irgendeinem Zeitpunkt bewusstlos geworden.\n Du hörst den Chef irgendwo rufen {Blau}'Es geht ihm gut, er war nur etwas müde, er soll wieder an die Arbeit.'{Reset}")
                                        D.clean_print(f"Zu tiefst beschämt und zugleich wütend, steigst du wieder hoch in den Verkaufsbereich, um deine Arbeit fortzusetzen.\nDeine Gedanken wirbeln durcheinander, und dir fallen hundert fiese Sprüche ein, wie du ihn kontern könntest.\nDoch keine Sekunde lang kannst du verschnaufen, da ruft eine Kundin'{Grün}\n\n")
                                        D.clean_print("Drücke eine beliebige Taste, um fortzufahren..")
                                        warte = input()
...
if Level == 2 and P[2] == 1:
    ...
    if D.X == 1: # Männer verfolgen
        ...
        D.clean_print(f"Du gehst direkt zum Büro des Filialleiters, stürmst hinein und stotterst:{Rot} 'Ich habe gerade zwei Männer auf der Toilette gehört, die über einen Koffer gesprochen haben. Es klang irgendwie verdächtig.' {Reset} \nWährend du überlegst, sagt er:{Rot} 'Danke für die Information, ich werde das im Auge behalten. Aber jetzt zurück an die Arbeit.' {Reset}\n\nInnerlich schreist du vor Wut und mangelndem Verständnis. Wie kann er das so ruhig hinnehmen. Möchtest du versuchen, Ihn zu überzeugen, sich der Sache JETZT zu stellen?[1=Ja],[2=Nein]")
        D.get_choice(2)
        if D.X == 1:
            ...
            if D.Redeerfolg == 1:
                ...
                D.clean_print("Du hast dein Bestes gegeben, jedoch hat es nicht geklappt, weshalb du das Büro verlässt und wieder an die Arbeit gehst.")
            D.Übergang()
        elif D.X == 2:
            D.clean_print("Schweigend verlässt du sein Zimmer, schließt hinter dir ab und gehst mit einem merkwürdigen Blick zurück zur Arbeit.")
    ...
    if Verbinder == True:
        ...
        if D.X == 1:
            ...
            if D.X == 1:
                ...
                D.clean_print(f"{Rot}'Was?? Ich?! Neinnn, ich wollte hier '")#Fortsetzung/////////////////////
        elif D.X == 2:
            D.clean_print(f"")#Fortsetzung///////////////////
