...
x = 0
erwischt = False
Raubwahrscheinlichkeit = 0
aktuellesObjekt = None
...
def Räuberinstinkt(): #nicht zuende ausgebaut
    erwischt = False
    while True:
        if aktuellesObjekt == "Handy":
            Mögliche_Erhöhung = (random.choice([4],[2],[2],[1],[1],[1],[1],[0.5],[0.5],[0.3],[0.3]))
            Raubwahrscheinlichkeit += Mögliche_Erhöhung





        if Raubwahrscheinlichkeit >= 10:
            print("Du wurdest erwischt.")
        else:
            print("Du konntest ein Handy stehlen.")
...
while Level < 10:
    while Level < 1:
        clear_screen()
        print("Es ist stockdunkel. Du erkennst nicht einmal deine eigene Hand. Doch plötzlich – ein flackernder Lichtschein! \nEine defekte LED an der Wand blitzt auf und wirft kaltes Licht in den Raum.Jetzt siehst du sie – \ndie Kiste mit der Aufschrift „iPhone-Lieferung“. Dein Herz schlägt schneller. \nNiemand darf dich sehen. Wirst du verschwinden, bevor es zu spät ist – oder die Sache durchziehen und dir dein \n„ehrliches Geld“ verdienen?")
        get_choice(2)
        if x == 1:
            ...
            if x == 1:
                    ...
                    Ausbruch = True  # Erreichen der While Schleife(Line54) mit break nicht möglich-> VARIABLE(Ausbruch) bildet "Faden"und bevor Line54 etwas ausführt, haben wir hier das Signal aktiviert, zu breaken  <- Kommentar kann gelöscht werden
                    #break #break wirft uns aus der while Schleife, weil sie unendlich lange gehen würde
            elif x == 2:
